class ApplicationController < ActionController::Base
  ...
  rescue_from ActionController::InvalidCrossOriginRequest, with: :bad_request if Rails.env.production?
  rescue_from ActionController::InvalidAuthenticityToken, with: :bad_request if Rails.env.production?
  rescue_from AbstractController::ActionNotFound, with: :route_not_found if Rails.env.production?
  rescue_from ActionView::MissingTemplate, with: :bad_request if Rails.env.production?
  rescue_from ActiveRecord::RecordNotFound, with: :resource_not_found if Rails.env.production?
  rescue_from ActiveRecord::RecordNotSaved, with: :not_acceptable if Rails.env.production?
  rescue_from ActionController::RoutingError, with: :route_not_found if Rails.env.production?
